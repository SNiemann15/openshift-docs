...
[NOTE]
====
NVIDIA GPU is supported on `x86` and `aarch64` architectures and IBM Spyre is supported on `s390x` architecture.
====
+
[source,terminal]
----
...
----
** `<gpu_accelerator>`: Valid values are `nvdia` and `spyre`.
